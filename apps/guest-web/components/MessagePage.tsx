import type { ReactNode } from "react";

// Shared shell for the single-message outcome pages (out-of-order, invalid,
// device-limit, post-stay, vacant, portal). Visual design is intentionally
// unstyled beyond spacing — full DHOP branding (Satoshi/Inter) lands once
// there's more than one screen worth designing against.
export function MessagePage({ title, children }: { title: string; children?: ReactNode }) {
  return (
    <main style={{ maxWidth: 480, margin: "4rem auto", padding: "0 1.5rem" }}>
      <h1 style={{ fontSize: "1.5rem", marginBottom: "1rem" }}>{title}</h1>
      {children}
    </main>
  );
}
